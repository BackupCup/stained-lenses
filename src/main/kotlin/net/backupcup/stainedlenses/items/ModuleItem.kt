...
class ModuleItem(settings: Settings?,
    ...
    override fun appendTooltip(
        ...
        if (InputUtil.isKeyPressed(MinecraftClient.getInstance().window.handle, boundSneakKey.code)) {
            tooltipList.forEach { text -> TextWrapUtils.wrapText((MinecraftClient.getInstance().window.width / 1.5f).toInt(), text, Formatting.GRAY).forEach { formattedText -> tooltip.add(formattedText) } }
        } else {
